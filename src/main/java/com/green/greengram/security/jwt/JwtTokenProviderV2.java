...
@Slf4j
@Component//컴포넌트는 빈 등록
public class JwtTokenProviderV2 {
    private final ObjectMapper om;
    private final AppProperties appProperties;
    private final SecretKey secretKey;//final 쓸거면 생성자에서 초기화 시켜야함
    ...
    public JwtTokenProviderV2(ObjectMapper om, AppProperties appProperties) {
        ...
        //암호화, 복호화 할 때 사용하는 키를 생성하는 부분, decode메소드에 보내는 아규먼트값은 우리가 설정한 문자열
    }
    ...
    private String generateToken(MyUser myUser, long tokenValidMilliSecond) {
        ...
        //메소드 호출 . 메소드 호출. 메소드 호출 >> 체이닝 기법, 원리는 메소드호출 시 자신의 주소값 리턴을 하기 때문
    ...
    public UserDetails getUserDetailsFromToken(String token) {
        try {
            ...
            String json = (String)claims.get("signedUser");//Claims에 저장되어 있는 값을 얻어온다.(그것이 JSON(데이터)) JWT도 문자열이라 헷갈릴 수 있음
            MyUser myUser = om.readValue(json, MyUser.class); //JSON >> 객체로 변환 (그것이 UserDetails, 정확
    ...
    public Authentication getAuthentication(String token){
        UserDetails userDetails = getUserDetailsFromToken(token); // 정확히는 MyUserDetails 객체주소값
    ...
    public boolean isValidateToken(String token){
        try {
            ...
            //!를 줘서(변환) 만료시간이 안 지났으면 리턴 true , 지났으면 리턴 false
        }catch (Exception e){
            return false;
        }
    }
...
//        String auth = req.getHeader("authorization"); 이렇게 작성한 것과 같음 key값은 변경가능
        ...
        //auth에는 "Bearer JWT"문자열이 있을 것이다 .문자열이 'Bearer'롤 시작하는지 체크 할거임
        //if(auth.startsWith("Bearer"))과 같음 //auth에 저장되어 있는 문자열이 "Bearer"로 시작하면  true , 아니면 false
        //FE와 약속을 만들어야함
        // authorization: Bearer JWT문자열
        if(!jwt.startsWith(appProperties.getJwt().getTokenType())){
            return null;
        }
        //순수한 JWT문자열만 뽑아내기 위한 문자열 자르기 + trim (양쪽 빈 칸 제거)
        return  jwt.substring(appProperties.getJwt().getTokenType().length()).trim();//trim은 빈 칸 없애주려고
                                                                    // (trim은 양쪽에 빈칸 있으면 없애줌 중간은 안됨)
    }
}